...
export default {
  ...
  methods: {
    initData: function () {
      let codec = new JsonCodec()
      codec.decode(demoData, this.graph.getModel())
      console.log('model', this.graph.model)
    },
    showFileText: function () {
      let encoder = new JsonCodec()
      let jsonObject = encoder.encode(this.graph.getModel())
